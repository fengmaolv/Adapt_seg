...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = './snapshots/model_baseline_dropout/GTA5_249999.pth'      ##########
SAVE_PRED_EVERY = 500
    ...
    for i_iter in range(args.num_steps):
       ...
       # loss_seg_value1 = 0
        ...
        for sub_i in range(args.iter_size):
            ...
            pred = model(images)    
            pred = interp(pred)
            pred_sta = pred[0].permute(1,2,0)
            pred_sta = torch.max(pred_sta, 2)[1].byte()
            pred_sta_mask = pred_sta[mask_weakly[0][18]==1]
           # print("amy", pred_sta[mask_weakly[0][0]==1])
            print("fengmao", torch.sum(pred_sta_mask==18))


            class_label_source_lse = class_label_source.type(torch.FloatTensor)
            num = torch.sum(mask_weakly[0][18]).data.item()
            lse  = (1.0/1) * torch.log((512*256/num)  * AvePool(torch.exp(1*pred) * mask_weakly.type(torch.FloatTensor).cuda(args.gpu)))
           # print(lse)  
            print(torch.max(pred[0][18]* mask_weakly.type(torch.FloatTensor).cuda(args.gpu)), torch.min(pred[0][18]* mask_weakly.type(torch.FloatTensor).cuda(args.gpu)),lse[0][18]) 
            loss_lse_source = bce_loss(lse[0][18], Variable(class_label_source_lse.reshape(lse.size())[0][18]).cuda(args.gpu))
            print(class_label_source_lse.reshape(lse.size())[0][18])
        ...
        #    loss_lse_target = bce_loss(lse[0][4], Variable(class_label_target_lse.reshape(lse.size())[0][4]).cuda(args.gpu))
            ...
            loss = 0.0  * loss_seg + 0.0 *  loss_weak + 0.0 * loss_neg + 1 *  loss_lse_source # + 0.00 * loss_lse_target# + 0.1 * entropy_samples
       ...
       # optimizer.step()
        ...
        print('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f} loss_weak = {3:.3f} loss_neg = {4:.3f} loss_lse_source = {5:.3f} loss_lse_target = {6:.3f} entropy_samples = {7:.3f}'.format(i_iter, args.num_steps, loss_seg_value, loss_weak_value, loss_neg_value, loss_lse_source_value, 0, entropy_samples_value))
