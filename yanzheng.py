...
DATA_LIST_PATH_TARGET = './dataset/cityscapes_list/train_mini.txt'
...
SAVE_PRED_EVERY = 500
SNAPSHOT_DIR = './snapshots/model_yanzheng'   ##########
RESULTS_DIR = './result_yanzheng.txt'                  ##########
WEIGHT_DECAY = 0.0005
...
############################
#validation data
    testloader = data.DataLoader(dataset.cityscapes_dataset.cityscapesDataSet(args.data_dir_target, args.data_list_target, crop_size=input_size_target, mean=IMG_MEAN, scale=False, mirror=False, set=args.set), batch_size=1, shuffle=False, pin_memory=True)
    with open('./dataset/cityscapes_list/info.json', 'r') as fp:
        info = json.load(fp)
    mapping = np.array(info['label2train'], dtype=np.int)
    label_path_list = './dataset/cityscapes_list/train_lable.txt'
    gt_imgs = open(label_path_list, 'r').read().splitlines()
    gt_imgs = [join('./data/Cityscapes/data/gtFine/train', x) for x in gt_imgs]
    ...
    for i_iter in range(args.num_steps):
       ...
       # loss_seg_value1 = 0
        ...
        for sub_i in range(args.iter_size):
            ...
            loss =  loss_seg + 0.2 * loss_lse_target
